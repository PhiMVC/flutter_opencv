...
class _CameraHomeState extends State<CameraHome> {
  ...
  static const double _embeddingThreshold = 0.45;
  ...
  Future<void> _addFromLibrary() async {
    ...
    try {
      ...
      await _galleryService.saveFromXFile(picked);
      await _loadTempImages();
    } catch (_) {
      // Ignore gallery pick errors.
    } finally {
      if (mounted) {
        setState(() {
          _isPicking = false;
  ...
  Future<void> _switchCamera() async {
    if (_isSwitchingCamera || _cameras.length < 2) {
      return;
    }
    setState(() {
      ...
      _lastNonEmptyDetections = const [];
      _lastNonEmptyImageSize = null;
    });
  ...
  Widget _buildCaptureBar() {
    ...
    final addLabel = _isPicking ? 'Đang thêm...' : 'Thêm ảnh';
    final canSwitchCamera = !_isSwitchingCamera && _cameras.length > 1;
            ...
            children: [
              Expanded(
                child: OutlinedButton.icon(
                  onPressed: _isPicking ? null : _addFromLibrary,
                  icon: const Icon(
